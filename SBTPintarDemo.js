...
const pengurus = [
  { nama: "Pak Chandra", jabatan: "Ketua RT", hp: "081210000001" },
    ...
    <div className="app-root" style={{ minHeight: "100vh", background: COLORS.bg, fontFamily: "'Inter', sans-serif", color: COLORS.ink, paddingBottom: 90 }}>
      <style>{`
        * { box-sizing: border-box; }
        ...
        .panic-btn { position: fixed; left: 50%; transform: translateX(-50%); bottom: 26px; z-index: 60; width: 64px; height: 64px; border-radius: 50%; background: ${COLORS.danger}; border: 4px solid #fff; color: #fff; display: flex; align-items: center; justify-content: center; box-shadow: 0 8px 22px rgba(214,64,46,0.4); cursor: pointer; transition: transform .12s; }
        .panic-btn.armed { transform: translateX(-50%) scale(1.12); box-shadow: 0 0 0 9px rgba(214,64,46,0.18), 0 8px 22px rgba(214,64,46,0.4); }
        ...
        .pay-ipl-btn { position: fixed; right: 22px; bottom: 24px; z-index: 55; }
      ...
      <div style={{ display: "flex" }}>
        ...
        {/* bottom nav (mobile) */}
        <nav className="bottomnav no-print" style={{ display: "none", position: "fixed", bottom: 0, left: 0, right: 0, background: COLORS.card, borderTop: `1px solid ${COLORS.divider}`, padding: "8px 4px", justifyContent: "space-around", zIndex: 10 }}>
          {tabs.map((t) => {
            const Icon = t.icon; const active = tab === t.id;
            return (
